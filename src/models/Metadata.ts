export type MetadataInput = string | Record<string, any> | Metadata | undefined | null;
...
function deepMerge(a: any, b: any): any {
  if (!isPlainObject(a) || !isPlainObject(b)) return isPlainObject(b) ? { ...b } : b;
  const out: Record<string, any> = { ...a };
  for (const [k, v] of Object.entries(b)) {
    if (k in out && isPlainObject(out[k]) && isPlainObject(v)) out[k] = deepMerge(out[k], v);
    else out[k] = v;
  }
  return out;
}
...
export class Metadata {
  ...
  private constructor(raw?: string, json?: any) {
    ...
    this._json = typeof json !== "undefined" ? json : (typeof this._raw !== "undefined" ? parseRawToJson(this._raw) : undefined);
  ...
  static fromRaw(raw: string, opts?: { parseJson?: boolean }): Metadata {
    const json = opts?.parseJson === false ? { text: raw } : parseRawToJson(raw);
    return new Metadata(raw, json);
  }

  static fromJson(json: any, opts?: { stringify?: boolean }): Metadata {
  ...
  static combine(
    ...
    if (leftJson && rightJson) mergedJson = strategy === "deep" ? deepMerge(leftJson, rightJson) : { ...leftJson, ...rightJson };
    ...
    else if (rawMode === "concat") raw = [left._raw, right._raw].filter(Boolean).join(opts?.delimiter ?? "\n");
    else raw = mergedJson !== undefined ? safeStringify(mergedJson) : (right._raw ?? left._raw);

    return new Metadata(raw, mergedJson);
  }

  hasRaw(): boolean { return typeof this._raw === "string"; }
  hasJson(): boolean { return typeof this._json !== "undefined"; }

  raw(): string | undefined { return this._raw; }
  ...
  text(): string { return this._raw ?? safeStringify(this._json); }
  toString(): string { return this.text(); }
  toJSON(): { raw?: string; json?: any } { return { raw: this._raw, json: this._json }; }
  ...
  withJson(json: any, stringify = true): Metadata {
    const nextRaw = stringify ? safeStringify(json) : (this._raw ?? safeStringify(json));
