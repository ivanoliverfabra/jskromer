...
import { get, mapResult, parseErrorMessage, post, resultErr, type Result } from "../utils";
...
export class Misc {
  static async getSupply(): Promise<Result<number>> {
    const res = await get<{ money_supply: number }>(`${KROMER_ENDPOINT}/supply`);
  ...
  static async login(pkey: PrivateKeyResolvable): Promise<Result<{ authed: boolean; address: string | null }>> {
    try {
      const res = await post<{ authed: boolean; address: string | null }>(`${KROMER_ENDPOINT}/login`, {
