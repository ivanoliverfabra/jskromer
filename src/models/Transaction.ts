...
import { resolvePrivateKey, resolveWallet, Wallet, type PrivateKeyResolvable, type WalletResolvable } from "./Wallet";
...
export class Transaction {
  ...
  withMetadata(meta: MetadataInput): Transaction {
    const copy = new Transaction(this.toJSON() as any);
  ...
  static async create(wallet: PrivateKeyResolvable, to: WalletResolvable, amount: number, metadata?: MetadataInput): Promise<Result<Transaction>> {
    try {
      ...
      const res = await post<{ transaction: TransactionType }>(`${KROMER_ENDPOINT}/transactions`, {
  ...
  static async get(id: number): Promise<Result<Transaction>> {
    const res = await get<{ transaction: TransactionType }>(`${KROMER_ENDPOINT}/transactions/${id}`);
    return mapResult(res, (v) => new Transaction(v.transaction));
  }

  static async list(unsafePagination?: Partial<Pagination>): Promise<Result<{ count: number; total: number; transactions: Transaction[] }>> {
    ...
    const res = await get<{ count: number; total: number; transactions: TransactionType[] }>(url.toString());
  ...
  static async listLatest(unsafePagination?: Partial<Pagination>): Promise<Result<{ count: number; total: number; transactions: Transaction[] }>> {
    const { limit, offset } = parsePagination(unsafePagination);
