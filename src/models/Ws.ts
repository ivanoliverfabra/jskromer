...
export type ErrorEventPayload<E = unknown> = { message?: string; error?: E };
...
export class WS {
	...
	private opts: Required<
		Pick<WSOptions, "autoReconnect" | "reconnectDelayMs">
	> &
		WSOptions;
	...
	static async start(privateKey?: PrivateKeyResolvable): Promise<string> {
		...
		if (typeof privateKey !== "undefined")
			body.privatekey = resolvePrivateKey(privateKey);
		const res = await post<{ url: string }>(
			`${KROMER_ENDPOINT}/ws/start`,
			body,
		);
	...
	static async connect(
		...
		const isUrl =
			typeof privateKeyOrUrl === "string" &&
			/^wss?:\/\//i.test(privateKeyOrUrl);
	...
	open(): void {
		const Ctor =
			...
			(globalThis as { WebSocket: new (...args: unknown[]) => WebSocketLike })
				.WebSocket;
		if (!Ctor)
			throw new Error(
				"No WebSocket implementation found. Provide options.WebSocketImpl.",
			);
		...
		ws.onmessage = (ev) => {
			...
			if (!data.ok())
				return console.error("Failed to parse WS message:", data.error());

			if (data && data.type === "keepalive") {
				if (typeof data.server_time === "string") this.emit("keepalive", data);
			} else if (
				data &&
				data.type === "event" &&
				...
				if (typeof data.responding_to === "string") {
					this.emit(`response:${data.responding_to}`, data as WSResponseBase);
				}
		...
		ws.onerror = (ev) => this.emit("error", this.normalizeErrorEvent(ev));
		ws.onclose = (ev) => {
	...
	subscribe(event: SubscriptionEvent | SubscriptionEvent[]): void {
		// 'id' is unused and kept for backward compatibility.
		if (Array.isArray(event)) {
			for (const ev of event) this.request({ type: "subscribe", event: ev });
		} else {
		...
		try {
			...
			const promise = new Promise<TRes>((resolve, reject) => {
				const timer = setTimeout(() => {
					...
					reject(
						new Error(
							`Request timed out: ${(message as WSRequest).type} (${id})`,
						),
					);
				}, timeoutMs);
				this.pending.set(id, {
	...
	on<T extends string>(
		event: Exclude<T, keyof WSEventMap>,
	...
	private emit<E extends keyof WSEventMap>(
		event: E,
		payload: WSEventMap[E],
	): void;
	private emit<T extends string>(
		event: Exclude<T, keyof WSEventMap>,
		payload: unknown,
	): void;
	...
	private normalizeErrorEvent(event: Event | CloseEvent): ErrorEventPayload {
	...
	private normalizeCloseEvent(ev: CloseEvent): CloseEventPayload {
		return {
			code: ev?.code,
			reason: ev?.reason,
